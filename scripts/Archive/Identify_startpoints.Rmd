...
id_pp<-function(x){
  ...
  dplyr::filter(NEXT_DOWN == HYRIV_ID) #%>% 
              #  ENDORHEIC != 1) %>% 
  st_cast(., "LINESTRING") %>% 
  filter(!duplicated(HYRIV_ID))
  
  pnts<-st_startpoint(sub_riv) 
  ...
  st_write(data_per_river, paste0(here("output_data/Pourpoints/"), "Startpoints_",name2, ".shp"), delete_layer =  TRUE)
