 ...
 # Match to sediment calculation shapefile for country
  ...
  dplyr::select(ISO3, Country, MAIN_BAS, Continent, sed_sum, sed_mean) %>% 
   ...
   pp_loc_done<-pp_loc %>% 
     filter(!is.na(ISO3) == T) %>% 
     as.data.frame() %>% 
     dplyr::select(-geometry, -sed_sum, -sed_mean)
   ...
   # TRY ALL OF THESE TO CORRECT SPECIAL CASES
  ...
  write.csv(done, here(paste0("output_data/Pourpoints/PP_location_", name2, ".csv")))
...
land_trans_sed<-function(x){
  ...
  #Check to make sure all basins have pp location data- should be 0
 ...
 # Transboundary basins withint pourpoints
 # There is just 1 now.
 test7<-test6 %>% filter(n>1)
 
 length(unique(test7$MAIN_BAS))
 
 # That leave us with 6 main basins that are transboundary but don't have pourpoint location
 ...
 # NEED TO MANUALLY FIX THESE 16 for Africa
...
land_trans_sed<-function(x){
  ...
  # Join sediment data to proportion o basin data
 ...
 pp_cont<-
  ...
  # This removes basins that have NO pourpoint identified - we add these back in later
  # AND country/basins that do not have pourpoints
