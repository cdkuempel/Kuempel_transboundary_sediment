...
# All pourpoints
pp_list<-list.files(here("output_data/Pourpoints/"), pattern = ".shp", full.names = T)

pp<-
  do.call(rbind,
          lapply(pp_list, st_read))
```
...
for(i in 1:length(bas)){
  ...
  n_iso3 <-length(unique(sub$ISO3))
    ...
    dplyr::select(ISO3, Country, MAIN_BAS, Continent, sed_sum, sed_mean, area_km2, b_area_km2, iso3_b_km2, prop_basin, n, total_sed, prop_sed, HYRIV_I, rv_d_3_, n_pp, total_pp, prop_pp) %>% 
